...
export default function AdminCozinha() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-stone-800 mb-6">Cozinha</h1>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {COLUNAS.map((col) => (
          <div key={col.key} className="rounded-xl bg-stone-100 p-4">
            <h3 className="font-semibold text-stone-700 mb-3">{col.label}</h3>
            <div className="space-y-3">
              {porColuna(col.key).map((p) => (
                <div key={p.id} className="rounded-lg bg-white p-3 shadow-sm border border-stone-200">
                  ...
                  <p className="text-sm text-stone-600">{p.cliente_nome || (p.comandas as any)?.nome_cliente || '-'}</p>
